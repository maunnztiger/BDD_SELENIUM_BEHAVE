...
class CreateButton(BasePage):
    def __init__(self, context):
        ...
        self.create_record_button_xpath = "/html/body/div[7]/div[2]/button[1]"
        self.close_editor_button_xpath = "/html/body/div[7]/div[2]/button[2]"
        self.inserted_row_xpath = "/html/body/div[4]/table/tbody/tr[5]"
    ...
    def verify_headline_text(self, popup_headline_text):   
        popup = self.libs.get_element_by_xpath(self.driver, self.popup_headline_xpath)
        headline_text= popup.text 
        assert headline_text == popup_headline_text
     
        sleep(1)
        
    def verify_id_label_text(self, id_label_text):
        id_label = self.libs.get_element_by_xpath(self.driver, self.id_label_xpath)       
        label_text = id_label.text
        assert label_text == id_label_text
        sleep(1)
        
    def verify_aspekt_label_text(self, aspekt_label_text):
        aspekt_label = self.libs.get_element_by_xpath(self.driver, self.aspekt_label_xpath)
        label_text = aspekt_label.text
        assert label_text == aspekt_label_text
        sleep(1)
    
    def verify_value_label_text(self, value_label_text):
        value_label = self.libs.get_element_by_xpath(self.driver, self.value_label_xpath)        
        label_text = value_label.text
        assert label_text == value_label_text
        sleep(1)
        
    def validate_create_button(self, create_button_text):
        create_record_button = self.libs.get_element_by_xpath(self.driver, self.create_record_button_xpath)    
        button_text = create_record_button.text
        assert button_text == create_button_text
        sleep(1)
        
    def validate_cancel_button(self, cancel_button_text):
        cancel_button = self.libs.get_element_by_xpath(self.driver, self.close_editor_button_xpath)    
        button_text = cancel_button.text
    ...
    def validate_new_rows_number(self):
        WebDriverWait(self.driver, 50).until(EC.presence_of_element_located((By.XPATH, self.table_body_xpath)))
        rows = self.libs.get_elements_by_xpath(self.driver, self.table_rows_xpath)
        assert len(rows) == (self.number_of_rows+1)
        sleep(1)
    
    def validate_last_row_id_column_text_entry(self):
        last_row_first_column = self.libs.get_element_by_xpath(self.driver, self.table_last_row_first_column_xpath)    
        row_text_content = last_row_first_column.text
        assert str(row_text_content) == str(self.number_of_rows)
        sleep(1)
    
    def validate_last_row_aspekt_column_text_entry(self, aspekt_text_entry):
        last_row_second_column = self.libs.get_element_by_xpath(self.driver, self.table_last_row_second_column_xpath) 
        row_text_content = last_row_second_column.text
        assert row_text_content == aspekt_text_entry
        sleep(1)
       
    def validate_last_row_value_column_text_entry(self, value_text_entry):
        last_row_third_column = self.libs.get_element_by_xpath(self.driver, self.table_last_row_third_column_xpath) 
        row_text_content = last_row_third_column.text
    ...
    def validate_deleted_rows_disapears(self):
        try:
            WebDriverWait(self.driver, 2).until_not(EC.presence_of_element_located((By.XPATH, self.inserted_row_xpath )))
